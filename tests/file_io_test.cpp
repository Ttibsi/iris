...
TEST_CASE("open_file", "[FILESYSTEM]") {
    ...
    const auto buf = Gapbuffer(contents);

    auto actual = open_file("tests/fixture/test_file_1.txt");

    REQUIRE(actual.has_value() == true);
    REQUIRE(actual.value() == buf);
    REQUIRE(actual.value().pos() == 0);
    REQUIRE(actual.value().to_str() == contents);
}
...
TEST_CASE("write_to_file", "[FILESYSTEM]") {
    Gapbuffer buf = Gapbuffer("Hello world");
    size_t bytes = write_to_file("save_test_file.txt", buf);
    REQUIRE(bytes == buf.size());

    auto contents = open_file("save_test_file.txt");
    REQUIRE(contents.has_value());
    REQUIRE(contents.value() == buf);
