...
Viewport::Viewport(Buffer *b, rawterm::Pos size)
    : buffer(b), action_controller(ActionController(this)), view_size(size), 
    cursor(Cursor(0)) {}

void Viewport::draw(const std::size_t &start_point) {
...
class NoLongerRelevantException : public std::exception {
    public:
        std::string what() { return "No longer used"; }
};

void Viewport::switch_to_insert() {
    throw NoLongerRelevantException();
...
void Viewport::switch_to_command() {
...
void Viewport::center(unsigned int line_num) {
    rawterm::clear_screen();
...
void Viewport::cursor_down(unsigned int count) {
...
void Viewport::cursor_up(unsigned int count) {
    for (unsigned int i = 0; i < count; i++) {
