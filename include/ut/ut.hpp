...
namespace boost::inline ext::ut::inline v2_1_1 {
    namespace utility {
        ...
        template <class R, class... TArgs> class function<R(TArgs...)> {
          public:
            ...
            constexpr function(function &&other) noexcept
                : invoke_{ static_cast<decltype(other.invoke_) &&>(
                      other.invoke_) },
                  destroy_{ static_cast<decltype(other.destroy_) &&>(
                      other.destroy_) },
          ...
          private:
            template <class T>
            [[nodiscard]] static auto invoke_impl(void *data, TArgs... args)
                -> R {
        ...
        [[nodiscard]] inline auto is_match(std::string_view input,
            ...
            if (pattern[0] != '?' and pattern[0] != '*' and
                pattern[0] != input[0]) {
                return false;
            }
        ...
        [[nodiscard]] inline auto split(T input, TDelim delim)
            -> std::vector<T> {
            std::vector<T> output{};
            std::size_t first{};
        ...
        constexpr auto regex_match(const char *str, const char *pattern)
            -> bool {
            if (*pattern == '\0' && *str == '\0')
                return true;
            if (*pattern == '\0' && *str != '\0')
...
// for over compiler need over macros
            ...
            inline constexpr const std::size_t raw_length =
            ...
            inline constexpr const std::size_t tail_length =
                raw_length - prefix_length;
            static_assert(need_length <= tail_length,
                          "Auto find prefix and suffix length broken error 4");
            inline constexpr const std::size_t suffix_length =
                tail_length - need_length;
        ...
        [[nodiscard]] constexpr auto type_name() -> const std::string_view {
            ...
            const std::size_t end =
                raw_type_name.length() - detail::suffix_length;
            const std::size_t len = end - detail::prefix_length;
            std::string_view result =
                raw_type_name.substr(detail::prefix_length, len);
            return result;
        }
    ...
    namespace type_traits {
        ...
        struct has_static_member_object_value<
            T, std::void_t<decltype(declval<T>().value)>>
            : std::bool_constant<
...
#else
        template <class From, class To>
        constexpr auto is_convertible(int)
    ...
    template <typename CharT, std::size_t SIZE> struct fixed_string {
        ...
        constexpr explicit(false)
            fixed_string(const CharT (&str)[N + 1]) noexcept {
            if constexpr (N != 0)
  ...
  {"--wait-for-keypress", "<never|start|exit|both>", std::ref(wait_for_keypress), "waits for a keypress before exiting"}
            ...
            static inline void parse_arg_with_fallback(int argc,
                                                       const char *argv[]) {
            ...
            static inline void parse(int argc, const char *argv[]) {
                const std::size_t n_args =
                    argc > 0 ? static_cast<std::size_t>(argc) : 0U;
                ...
                for (auto i = 1U; i < n_args && argv != nullptr; i++) {
                    ...
                    const bool has_option_arg =
                        !std::get<1>(*cmd_option).empty();
                    if (!has_option_arg &&
                        std::holds_alternative<std::reference_wrapper<bool>>(
                            var)) {
                        std::get<std::reference_wrapper<bool>>(var).get() =
                            true;
                        continue;
                    }
                    if ((i + 1) >= n_args) {
                    ...
                    if (std::holds_alternative<
                            std::reference_wrapper<std::size_t>>(var)) {
                        // parse size argument
                        std::size_t last;
                        std::string argument(argv[i]);
                        std::size_t val = std::stoull(argument, &last);
                        if (last != argument.length()) {
                            std::cerr << "cannot parse option of "
                                      << argv[i - 1] << " " << argv[i]
                                      << std::endl;
                            std::exit(-1);
                        }
                        std::get<std::reference_wrapper<std::size_t>>(var)
                            .get() = val;
                    }
                    if (std::holds_alternative<
                            std::reference_wrapper<std::string>>(var)) {
                        // parse string argument
                        std::get<std::reference_wrapper<std::string>>(var)
                            .get() = argv[i];
                        continue;
                    }
                }
        ...
        template <class T>
        [[nodiscard]] constexpr auto get_impl(const T &t, int)
            -> decltype(t.get()) {
            return t.get();
        }
        template <class T>
        [[nodiscard]] constexpr auto get_impl(const T &t, ...)
            -> decltype(auto) {
            return t;
        }
        ...
        template <class T, class = int> struct value : op {
            ...
            constexpr /*explicit(false)*/ value(const T &_value)
                : value_{ _value } {}
        ...
        template <class T>
        struct value<
            T, type_traits::requires_t<type_traits::is_floating_point_v<T>>>
            : op {
            using value_type = T;
            ...
            constexpr approx_(const TLhs &lhs = {}, const TRhs &rhs = {},
                ...
                : lhs_{ lhs }, rhs_{ rhs }, epsilon_{ epsilon }, value_{ [&] {
                      ...
                      if constexpr (
                          ...
                          return math::abs_diff(get(lhs), get(rhs)) <
                                 get(epsilon);
                      }
                  }() } {}
    ...
    class printer {
      ...
      public:
        ...
        auto &operator<<(const detail::or_<TLhs, TRhs> &op) {
            return (*this << '(' << op.lhs() << color(op) << " or "
                          << colors_.none << op.rhs() << ')');
        }
      ...
      public:
        ...
        auto on(events::assertion_fail<TExpr> assertion) -> void {
            ...
            printer_ << "\n  " << short_name(assertion.location.file_name())
                     ...
                     << printer_.colors().fail << "FAILED"
                     << printer_.colors().none << " [" << std::boolalpha
    ...
    template <class TPrinter = printer> class reporter_junit {
        ...
        void check_for_scope(std::string_view test_name) {
            ...
            const auto [iter, inserted] =
                active_scope_->nested_tests->try_emplace(
                    str_name,
                    test_result{ active_scope_, detail::cfg::executable_name,
            ...
            if (!inserted) {
                std::cout << "WARNING test '" << str_name
                          << "' for test suite '" << active_suite_
                          << "' already present\n";
            }
        }
        ...
        void pop_scope(std::string_view test_name_sv) {
            ...
            ss << "not popping because '" << active_test_.top()
      ...
      public:
        ...
        auto on(events::run_begin run) {
            ::boost::ut::detail::cfg::parse_arg_with_fallback(run.argc,
                                                              run.argv);
        ...
        auto on(events::test_begin test_event)
            -> void { // starts outermost test
        ...
        auto on(events::test_finish test_event)
            -> void { // finishes nested test
            on(events::test_end{ .type = test_event.type,
                                 .name = test_event.name });
        }

        auto on(events::test_skip test_event) -> void {
            ss_out_.clear();
            if (!active_scope_->nested_tests->contains(
                    std::string(test_event.name))) {
                check_for_scope(test_event.name);
                active_scope_->status = "SKIPPED";
        ...
        auto on(events::exception exception) -> void {
            ...
            if (detail::cfg::abort_early ||
                ...
                std::cerr << "early abort for test : " << active_test_.top()
                          << "after ";
                std::cerr << active_scope_->fails << " failures total."
                          << std::endl;
                std::exit(-1);
            }
        }
      ...
      protected:
            ...
            for (const auto &[suite_name, suite_result] : results_) {
                if (suite_result.fails) {
                    err_stream << "\n=========================================="
                               ...
                               << " | " << color_.fail << suite_result.fails
                               << " failed" << color_.none << '\n';
                    std::cerr << std::endl;
                } else {
        ...
        void print_result(std::ostream &stream, const std::string &suite_name,
            ...
            for (const auto &[name, result] : *parent.nested_tests) {
                ...
                if (result.report_string.empty() &&
                    result.nested_tests->empty()) {
                    stream << " />\n";
    ...
    class runner {
        class filter {
          ...
          public:
            ...
            constexpr auto operator()(const std::size_t level,
                ...
                for (auto i = 0u;
                     i < math::min_value(level + 1, std::size(path_)); ++i) {
      ...
      public:
        ...
        template <class... Ts> auto on(events::test<Ts...> test) {
            ...
            for (const auto &tag_element : test.tag) {
                if (utility::is_match(tag_element, "skip") &&
                    !detail::cfg::show_tests && !detail::cfg::show_test_names) {
            ...
            if (filter_(level_, path_)) {
                if (not level_++) {
                    reporter_.on(
                        events::test_begin{ .type = test.type,
                                            .name = test.name,
                                            .location = test.location });
                } else {
                    reporter_.on(events::test_run{ .type = test.type,
                                                   .name = test.name });
                }
                ...
                if (not--level_) {
                    reporter_.on(events::test_end{ .type = test.type,
                                                   .name = test.name });
        ...
        template <class... Ts> auto on(events::skip<Ts...> test) {
            reporter_.on(
                events::test_skip{ .type = test.type, .name = test.name });
        }
        ...
        [[nodiscard]] auto run(run_cfg rc = {}) -> bool {
            ...
            for (const auto &[suite, suite_name] : suites_) {
                // add reporter in/out
                if constexpr (requires {
                                  reporter_.on(events::suite_begin{});
                              }) {
                    reporter_.on(events::suite_begin{ .type = "suite",
                                                      .name = suite_name });
                }
                suite();
                if constexpr (requires { reporter_.on(events::suite_end{}); }) {
                    reporter_.on(events::suite_end{ .type = "suite",
                                                    .name = suite_name });
                }
            }
            suites_.clear();
        ...
        template <class TExpr> class terse_ {
          public:
            ...
            ~terse_() noexcept(false) {
                ...
                void(detail::on<TExpr>(events::assertion<TExpr>{
    ...
    namespace literals {
        [[nodiscard]] inline auto operator""_test(const char *name,
                                                  std::size_t size) {
            return detail::test{
                "test", std::string_view{ name, size }
            };
        }
        ...
        constexpr auto operator""_b(const char *name,
    ...
    namespace operators {
        ...
        [[nodiscard]] constexpr auto operator==(T &&lhs, T &&rhs) {
            return detail::eq_{ static_cast<T &&>(lhs),
                                static_cast<T &&>(rhs) };
        }
        ...
        template <class T,
                  type_traits::requires_t<type_traits::is_range_v<T>> = 0>
        [[nodiscard]] constexpr auto operator!=(T &&lhs, T &&rhs) {
            return detail::neq_{ static_cast<T &&>(lhs),
                                 static_cast<T &&>(rhs) };
        }
        ...
        [[nodiscard]] constexpr auto operator==(const TLhs &lhs,
                                                const TRhs &rhs) {
            return detail::eq_{ lhs, rhs };
        }
        ...
        template <class TLhs, class TRhs,
                  type_traits::requires_t<type_traits::is_op_v<TLhs> or
                                          type_traits::is_op_v<TRhs>> = 0>
        [[nodiscard]] constexpr auto operator and(const TLhs &lhs,
                                                  const TRhs &rhs) {
        ...
        template <class TLhs, class TRhs,
                  type_traits::requires_t<type_traits::is_op_v<TLhs> or
                                          type_traits::is_op_v<TRhs>> = 0>
        [[nodiscard]] constexpr auto operator or(const TLhs &lhs,
                                                 const TRhs &rhs) {
...
#pragma clang diagnostic ignored "-Wunused-comparison"
            ...
            template <class T,
                      type_traits::requires_t<type_traits::is_op_v<T>> = 0>
            constexpr auto operator==(
                const T &lhs,
                const detail::value_location<typename T::value_type> &rhs) {
                using eq_t =
                    detail::eq_<T,
                                detail::value_location<typename T::value_type>>;
            ...
            constexpr auto operator==(
                ...
                const T &rhs) {
                using eq_t =
                    detail::eq_<detail::value_location<typename T::value_type>,
                                T>;
                struct eq_ : eq_t, detail::log {
                    using type = eq_t;
                    using eq_t::eq_t;
            ...
            constexpr auto operator!=(
                const T &lhs,
                const detail::value_location<typename T::value_type> &rhs) {
                using neq_t = detail::neq_<
                    T, detail::value_location<typename T::value_type>>;
            ...
            constexpr auto operator!=(
                ...
                const T &rhs) {
                using neq_t =
                    detail::neq_<detail::value_location<typename T::value_type>,
                                 T>;
                struct neq_ : neq_t {
                    using type = neq_t;
                    using neq_t::neq_t;
            ...
            operator>(const detail::value_location<typename T::value_type> &lhs,
                      const T &rhs) {
                using gt_t =
                    detail::gt_<detail::value_location<typename T::value_type>,
                                T>;
                struct gt_ : gt_t, detail::log {
                    using type = gt_t;
                    using gt_t::gt_t;
            ...
            constexpr auto operator>=(
                ...
                const T &rhs) {
                using ge_t =
                    detail::ge_<detail::value_location<typename T::value_type>,
                                T>;
                struct ge_ : ge_t, detail::log {
                    using type = ge_t;
                    using ge_t::ge_t;
            ...
            operator<(const detail::value_location<typename T::value_type> &lhs,
                      const T &rhs) {
                using lt_t =
                    detail::lt_<detail::value_location<typename T::value_type>,
                                T>;
                struct lt_ : lt_t, detail::log {
                    using type = lt_t;
                    using lt_t::lt_t;
            ...
            constexpr auto operator<=(
                ...
                const T &rhs) {
                using le_t =
                    detail::le_<detail::value_location<typename T::value_type>,
                                T>;
                struct le_ : le_t {
                    using type = le_t;
                    using le_t::le_t;
            ...
            constexpr auto operator and(const TLhs &lhs, const TRhs &rhs) {
                using and_t =
                    detail::and_<typename TLhs::type, typename TRhs::type>;
            ...
            constexpr auto operator or(const TLhs &lhs, const TRhs &rhs) {
                using or_t =
                    detail::or_<typename TLhs::type, typename TRhs::type>;
    ...
    template <fixed_string suite_name = "unnamed suite"> struct suite {
        ...
        constexpr /*explicit(false)*/ suite(TSuite _suite) {
            ...
            detail::on<decltype(+_suite)>(events::suite<decltype(+_suite)>{
                .run = +_suite, .name = name });
        }
    };
    ...
    namespace bdd {
        ...
        namespace gherkin {
            class steps {
                ...
                using call_step_t =
                    utility::function<void(const std::string &)>;
                using call_steps_t =
                    std::vector<std::pair<step_t, call_step_t>>;
                    ...
                    auto operator=(const TExpr &expr) -> void {
                        ...
                        steps_.call_steps().emplace_back(
                            ...
                            [expr, pattern = pattern_](const auto &_step) {
                                [=]<class... TArgs>(
                                    type_traits::list<TArgs...>) {
                                    log << _step;
                                    auto i = 0u;
                                    const auto &ms =
                                        utility::match(pattern, _step);
                                    expr(lexical_cast<TArgs>(ms[i++])...);
                                }(typename type_traits::function_traits<
                                    TExpr>::args{});
                            });
                    }
              ...
              public:
                ...
                constexpr /*explicit(false)*/ steps(const TSteps &_steps)
                    : steps_{ _steps } {}
              ...
              private:
                ...
                auto next(const TPattern &pattern) -> void {
                    ...
                    const auto call_steps = [this,
                                             is_scenario](const auto &_step,
                                                          const auto i) {
