 ...
 */
    ...
    auto safe_duration_cast(std::chrono::duration<FromRep, FromPeriod> from,
        ...
        ec = 0;
        // the basic idea is that we need to convert from count() in the from
        // type to count() in the To type, by multiplying it with this:
        struct Factor
        ...
        // overflow/underflow. let's start by finding a suitable type that can
        // hold both To, From and Factor::num
        ...
        if (detail::const_check(Factor::num != 1)) {
            ...
            if (detail::const_check(
                    !std::is_unsigned<IntermediateRep>::value) &&
                count < min1) {
                ec = 1;
                return {};
            }
        ...
        if (detail::const_check(Factor::den != 1))
            count /= Factor::den;
        auto tocount =
            lossless_integral_conversion<typename To::rep>(count, ec);
...
namespace detail {
    ...
    template <typename Streambuf> class formatbuf : public Streambuf {
      ...
      protected:
        // The put area is always empty. This makes the implementation simpler
        // and has the advantage that the streambuf and the buffer are always in
        // sync and sputc never writes into uninitialized memory. A disadvantage
        // is that each call to sputc always results in a (virtual) call to
        // overflow. There is no disadvantage here for sputn since this always
...
#pragma clang diagnostic ignored "-Wdeprecated"
        auto &f =
            std::use_facet<std::codecvt<CodeUnit, char, std::mbstate_t>>(loc);
#pragma clang diagnostic pop
    ...
    auto write_encoded_tm_str(OutputIt out, string_view in,
        ...
        if (detail::use_utf8() && loc != get_classic_locale()) {
            // char16_t and char32_t codecvts are broken in MSVC (linkage
            // errors) and gcc-4.
#if FMT_MSC_VERSION != 0 ||                                                    \
    (defined(__GLIBCXX__) &&                                                   \
     (!defined(_GLIBCXX_USE_DUAL_ABI) || _GLIBCXX_USE_DUAL_ABI == 0))
            // The _GLIBCXX_USE_DUAL_ABI macro is always defined in libstdc++
    ...
    auto write(OutputIt out, const std::tm &time, const std::locale &loc,
        ...
        do_write<char>(buf, time, loc, format, modifier);
        return write_encoded_tm_str(out, string_view(buf.data(), buf.size()),
                                    loc);
    }
    ...
    struct is_same_arithmetic_type
        : public std::integral_constant<
              bool, (std::is_integral<Rep1>::value &&
    ...
    auto fmt_duration_cast(std::chrono::duration<FromRep, FromPeriod> from)
        -> To {
#if FMT_SAFE_DURATION_CAST
    ...
    auto to_time_t(
        ...
        -> std::time_t {
        // Cannot use std::chrono::system_clock::to_time_t since this would
        // first require a cast to std::chrono::system_clock::time_point, which
        // could overflow.
...
namespace detail {
    ...
    inline void write_digit2_separated(char *buf, unsigned a, unsigned b,
        ...
        // We have x = a * 10 + b and we want to convert it to BCD y = a * 16 +
        // b. The difference is
        //   y - x = a * 6
    ...
    inline auto tm_wday_short_name(int wday) -> const char * {
        static constexpr const char *short_name_list[] = { "Sun", "Mon", "Tue",
                                                           "Wed", "Thu", "Fri",
                                                           "Sat" };
    ...
    // the C++20 spec. If more than 18 fractional digits are required then
    // returns 6 for microseconds precision.
    ...
    class tm_writer {
      private:
        ...
        // POSIX and the C Standard are unclear or inconsistent about what %C
        // and %y do if the year is negative or exceeds 9999. Use the convention
        // that %C concatenated with %y yields the same output as %Y, and that
        // %Y contains at least 4 characters, with more only if necessary.
        ...
        auto iso_year_weeks(long long curr_year) const noexcept -> int {
            ...
            const auto curr_p = (curr_year + curr_year / 4 - curr_year / 100 +
                                 curr_year / 400) %
                                days_per_week;
            const auto prev_p = (prev_year + prev_year / 4 - prev_year / 100 +
                                 prev_year / 400) %
                                days_per_week;
        ...
        template <typename T,
                  FMT_ENABLE_IF(has_member_data_tm_gmtoff<T>::value)>
        ...
        void on_datetime(numeric_system ns) {
            if (is_classic_) {
                ...
                *out_++ = ' ';
                ...
                format_localized('c',
                                 ns == numeric_system::standard ? '\0' : 'E');
            }
        }
            ...
            if (is_classic_ || ns == numeric_system::standard) {
                ...
                if (subsecs_) {
                    if (std::is_floating_point<typename Duration::rep>::value) {
                        ...
                        if (buf.size() > 1) {
                            // Remove the leading "0", write something like
                            // ".123".
    ...
    auto format_duration_value(OutputIt out, Rep val, int precision)
        ...
        specs.type = precision >= 0 ? presentation_type::fixed
    ...
    auto copy_unit(string_view unit, OutputIt out, wchar_t) -> OutputIt {
        // This works when wchar_t is UTF-32 because units only contain
        // characters that have the same representation in UTF-16 and UTF-32.
    ...
    struct chrono_formatter {
        ...
        // These are not implemented because durations don't have date
        // information.
        void on_abbr_weekday() {}
        void on_full_weekday() {}
