 ...
 */
    ...
    auto safe_duration_cast(std::chrono::duration<FromRep, FromPeriod> from, int& ec) -> To {
        ...
        using IntermediateRep = typename std::common_type<
            typename From::rep, typename To::rep, decltype(Factor::num)>::type;
...
#pragma clang diagnostic pop
        ...
        auto result = f.in(
            mb, in_buf.begin(), in_buf.end(), from_next, std::begin(out.buf), std::end(out.buf),
            out.end);
...
namespace detail {
    ...
    void write_fractional_seconds(OutputIt& out, Duration d, int precision = -1) {
        ...
        if (precision < 0) {
            ...
            if (std::ratio_less<
                    typename subsecond_precision::period, std::chrono::seconds::period>::value) {
                *out++ = '.';
    ...
    class tm_writer {
       ...
       public:
        ...
        void on_iso_date() {
            ...
            write_digit2_separated(
                buf + 2, static_cast<unsigned>(year % 100), to_unsigned(tm_mon() + 1),
                to_unsigned(tm_mday()), '-');
...
struct formatter<std::chrono::time_point<std::chrono::utc_clock, Duration>, Char>
    ...
    template <typename FormatContext>
    auto format(std::chrono::time_point<std::chrono::utc_clock, Duration> val, FormatContext& ctx)
        const -> decltype(ctx.out()) {
        return formatter<std::chrono::time_point<std::chrono::system_clock, Duration>, Char>::
