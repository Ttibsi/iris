...
#define FMT_USE_INT128 0
    ...
    FMT_CONSTEXPR auto to_unsigned(Int value) -> make_unsigned_t<Int> {
        FMT_ASSERT(std::is_unsigned<Int>::value || value >= 0,
                   "negative value");
    ...
    template <typename T>
    struct is_std_string_like<T,
                              void_t<decltype(std::declval<T>().find_first_of(
    ...
    struct is_back_insert_iterator<
        It, bool_constant<std::is_same<decltype(adl::invoke_back_inserter<
                                                typename It::container_type>()),
                                       It>::value>> : std::true_type {};

    // Extracts a reference to the container from *insert_iterator.
    template <typename OutputIt>
    inline auto get_container(OutputIt it) ->
...
FMT_DEPRECATED FMT_NORETURN inline void
...
namespace detail {
    ...
    class compile_parse_context : public basic_format_parse_context<Char> {
      ...
      public:
        ...
        compile_parse_context(basic_string_view<Char> format_str, int num_args,
            ...
            : base(format_str, next_arg_id), num_args_(num_args),
              types_(types) {}
    ...
    class iterator_buffer<
      ...
      private:
        ...
        static FMT_CONSTEXPR void grow(buffer<value_type> &buf,
                                       size_t capacity) {
...
namespace detail {
    ...
    template <typename T> struct format_as_result {
        template <typename U, FMT_ENABLE_IF(std::is_enum<U>::value ||
...
#define FMT_MAP_API FMT_CONSTEXPR FMT_ALWAYS_INLINE
    ...
    template <typename Context> struct arg_mapper {
        ...
        // Use SFINAE instead of a const T* parameter to avoid a conflict with
        // the array overload.
        template <
            typename T,
        ...
        FMT_MAP_API auto map(const T &val)
    ...
    FMT_CONSTEXPR auto make_arg(T &val) -> value<Context> {
        using arg_type =
            remove_cvref_t<decltype(arg_mapper<Context>().map(val))>;
        ...
        static_assert(formattable_char,
    ...
    void init_named_arg(named_arg_info<Char> *, int &arg_index, int &,
                        const T &) {
        ++arg_index;
    }
    template <typename Char, typename T, FMT_ENABLE_IF(is_named_arg<T>::value)>
    ...
    // `fmt::basic_format_args` for passing into type-erased formatting
    // functions such as `fmt::vformat`.
    ...
    struct format_arg_store {
        ...
        static constexpr size_t ARGS_ARR_SIZE =
            1 + (NUM_ARGS != 0 ? NUM_ARGS : +1);
        ...
        FMT_MAP_API format_arg_store(T &...values)
            ...
            (void)dummy{ 0, (init_named_arg(named_args, arg_index,
                                            named_arg_index, values),
                             0)... };
        }
  ...
  public:
    ...
    FMT_CONSTEXPR FMT_INLINE auto visit(Visitor &&vis) const
        ...
        case detail::type::custom_type:
            return vis(
                typename basic_format_arg<Context>::handle(value_.custom));
        }
...
namespace detail {
    ...
    FMT_CONSTEXPR auto code_point_length(const Char *begin) -> int {
        ...
        return static_cast<int>((0x3a55000000000000ull >> (2 * (c >> 3))) &
                                0x3) +
               1;
    }
    ...
    template <>
    inline auto find<false, char>(const char *first, const char *last,
                                  char value, const char *&out) -> bool {
    ...
    FMT_CONSTEXPR auto parse_format_specs(const Char *begin, const Char *end,
        ...
        for (;;) {
            ...
            case '{':
                ...
                begin = parse_dynamic_spec(begin, end, specs.width,
                                           specs.width_ref, ctx);
                break;
            case '.':
                if (arg_type == type::none_type)
            ...
            default: {
                ...
                specs.fill = basic_string_view<Char>(
    ...
    FMT_CONSTEXPR auto get_arg_index_by_name(basic_string_view<Char> name)
        -> int {
#if FMT_USE_NONTYPE_TEMPLATE_ARGS
        if constexpr (sizeof...(Args) > 0)
    ...
    template <typename Char, typename... Args> class format_string_checker {
      private:
        ...
        parse_func
            parse_funcs_[num_args > 0 ? static_cast<size_t>(num_args) : 1];

      public:
        explicit FMT_CONSTEXPR
        format_string_checker(basic_string_view<Char> fmt)
            : types_{ mapped_type_constant<Args,
                                           buffered_context<Char>>::value... },
...
#else
            (void)id;
            on_error("compile-time checks for named arguments require C++20 "
                     "support");
            return 0;
#endif
        }
    ...
    template <typename T, typename Char, type TYPE> struct native_formatter {
      ...
      public:
        ...
        template <type U = TYPE, FMT_ENABLE_IF(U == type::string_type ||
                                               U == type::cstring_type ||
                                               U == type::char_type)>
