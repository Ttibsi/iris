   ...
   public:
    ...
    basic_string_view(const Char* s)
        : data_(s),
          size_(
              detail::const_check(
                  std::is_same<Char, char>::value && !detail::is_constant_evaluated(false))
        ...
        for (;;) {
            switch (c) {
                ...
                case '.':
                    if (arg_type == type::none_type) return begin;
                    enter_state(
                        state::precision, in(arg_type, float_set | string_set | cstring_set));
    ...
    struct native_formatter {
       ...
       public:
        ...
        template <
            type U = TYPE,
            FMT_ENABLE_IF(
                U == type::string_type || U == type::cstring_type || U == type::char_type)>
...
#if FMT_USE_CONSTEVAL
        if constexpr (
            detail::count_named_args<Args...>() == detail::count_statically_named_args<Args...>()) {
