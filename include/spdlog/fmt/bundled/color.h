...
namespace detail {
    ...
    template <typename Char> struct ansi_color_escape {
        FMT_CONSTEXPR ansi_color_escape(detail::color_type text_color,
            ...
            // If we have a terminal color, we need to output another escape
            // code sequence.
            if (!text_color.is_rgb) {
                bool is_background = esc == string_view("\x1b[48;2;");
                uint32_t value = text_color.value.term_color;
                // Background ASCII codes are the same as the foreground ones
                // but with 10 more.
                if (is_background)
                    value += 10u;
    ...
    template <typename Char>
    void vformat_to(
        buffer<Char> &buf, const text_style &ts,
