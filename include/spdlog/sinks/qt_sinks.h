...
namespace spdlog {
    namespace sinks {
        ...
        // Color location is determined by the sink log pattern like in the rest
        // of spdlog sinks. Colors can be modified if needed using
        // sink->set_color(level, qtTextCharFormat). max_lines is the maximum
        // number of lines that the sink will hold before removing the oldest
        // lines. By default, only ascii (latin1) is supported by this sink. Set
        // is_utf8 to true if utf8 support is needed.
        ...
        class qt_color_sink : public base_sink<Mutex> {
          public:
            ...
            void set_level_color(level::level_enum color_level,
                                 QTextCharFormat format) {
                // std::lock_guard<Mutex> lock(base_sink<Mutex>::mutex_);
          ...
          protected:
            struct invoke_params {
                invoke_params(int max_lines, QTextEdit *q_text_edit,
                              QString payload, QTextCharFormat default_color,
            ...
            void sink_it_(const details::log_msg &msg) override {
                ...
                if (is_utf8_) {
                    payload = QString::fromUtf8(str.data(),
                                                static_cast<int>(str.size()));
                    // convert color ranges from byte index to character index.
                    if (msg.color_range_start < msg.color_range_end) {
                        color_range_start =
                            QString::fromUtf8(str.data(), msg.color_range_start)
                                .size();
                        color_range_end =
                            QString::fromUtf8(str.data(), msg.color_range_end)
                                .size();
                    }
                } else {
                    payload = QString::fromLatin1(str.data(),
                                                  static_cast<int>(str.size()));
                }
            ...
            // Add colored text to the text edit widget. This method is invoked
            // in the GUI thread. It is a static method to ensure that it is
            // handled correctly even if the sink is destroyed prematurely
            ...
            static void invoke_method_(invoke_params params) {
                ...
                // if color range not specified or not not valid, just append
                // the text with default color
                ...
                cursor.insertText(
                    params.payload.left(params.color_range_start));
...
#include <mutex>
    ...
    template <typename Factory = spdlog::synchronous_factory>
    inline std::shared_ptr<logger>
    qt_logger_mt(const std::string &logger_name, QObject *qt_object,
    ...
    template <typename Factory = spdlog::synchronous_factory>
    inline std::shared_ptr<logger>
    qt_logger_st(const std::string &logger_name, QObject *qt_object,
                 const std::string &meta_method) {
        return Factory::template create<sinks::qt_sink_st>(
            logger_name, qt_object, meta_method);
    }
