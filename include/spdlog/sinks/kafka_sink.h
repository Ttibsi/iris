...
namespace spdlog {
        ...
        template <typename Mutex> class kafka_sink : public base_sink<Mutex> {
          public:
            kafka_sink(kafka_sink_config config)
                ...
                try {
                    ...
                    RdKafka::Conf::ConfResult confRes = conf_->set(
                        "bootstrap.servers", config_.server_addr, errstr);
                    if (confRes != RdKafka::Conf::CONF_OK) {
                        throw_spdlog_ex(fmt_lib::format(
                            "conf set bootstrap.servers failed err:{}",
                            errstr));
                    }
                    ...
                    if (tconf_ == nullptr) {
                        throw_spdlog_ex(
                            fmt_lib::format("create topic config failed"));
                    }
                    ...
                    if (producer_ == nullptr) {
                        throw_spdlog_ex(fmt_lib::format(
                            "create producer failed err:{}", errstr));
                    }
                    ...
                    if (topic_ == nullptr) {
                        throw_spdlog_ex(fmt_lib::format(
                            "create topic failed err:{}", errstr));
                    }
                } catch (const std::exception &e) {
                    throw_spdlog_ex(fmt_lib::format(
                        "error create kafka instance: {}", e.what()));
                }
            }
          ...
          protected:
            void sink_it_(const details::log_msg &msg) override {
                producer_->produce(
                    topic_.get(), 0, RdKafka::Producer::RK_MSG_COPY,
    ...
    kafka_logger_mt(const std::string &logger_name,
        ...
        return Factory::template create<sinks::kafka_sink_mt>(logger_name,
                                                              config);
    }
    ...
    kafka_logger_st(const std::string &logger_name,
                    spdlog::sinks::kafka_sink_config config) {
        return Factory::template create<sinks::kafka_sink_st>(logger_name,
                                                              config);
    }
