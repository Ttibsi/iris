...
namespace spdlog {
    ...
    namespace sinks {
        ...
        stdout_sink_base<ConsoleMutex>::stdout_sink_base(FILE *file)
            ...
            handle_ =
                reinterpret_cast<HANDLE>(::_get_osfhandle(::_fileno(file_)));
            ...
            if (handle_ == INVALID_HANDLE_VALUE && file != stdout &&
                file != stderr) {
                throw_spdlog_ex(
        ...
        stdout_sink_base<ConsoleMutex>::log(const details::log_msg &msg) {
            ...
            bool ok = ::WriteFile(handle_, formatted.data(), size,
            ...
            memory_buf_t formatted;
            formatter_->format(msg, formatted);
            details::os::fwrite_bytes(formatted.data(), formatted.size(),
                                      file_);
#endif                       // _WIN32
