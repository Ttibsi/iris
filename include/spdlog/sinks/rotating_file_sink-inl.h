...
namespace spdlog {
    namespace sinks {
        ...
        SPDLOG_INLINE rotating_file_sink<Mutex>::rotating_file_sink(
            filename_t base_filename, std::size_t max_size,
            ...
            if (max_files > 200000) {
                throw_spdlog_ex("rotating sink constructor: max_files arg "
        ...
        SPDLOG_INLINE filename_t rotating_file_sink<Mutex>::calc_filename(
            ...
            return fmt_lib::format(
                SPDLOG_FMT_STRING(SPDLOG_FILENAME_T("{}.{}{}")), basename,
                index, ext);
        }
        ...
        rotating_file_sink<Mutex>::sink_it_(const details::log_msg &msg) {
            ...
            // (see issue #2261). we only check the real size when new_size >
            // max_size_ because it is relatively expensive.
        ...
        SPDLOG_INLINE void rotating_file_sink<Mutex>::rotate_() {
            ...
            for (auto i = max_files_; i > 0; --i) {
                ...
                if (!rename_file_(src, target)) {
                    ...
                    // this is a workaround to a windows issue, where very high
                    // rotation rates can cause the rename to fail with
                    // permission denied (because of antivirus?).
                    details::os::sleep_for_millis(100);
                    if (!rename_file_(src, target)) {
                        file_helper_.reopen(
                            true); // truncate the log file anyway to prevent it
