    ...
    namespace details {
          ...
          public:
            ...
            ~scoped_padder() {
                if (remaining_pad_ >= 0) {
                    ...
                    long new_size =
                        static_cast<long>(dest_.size()) + remaining_pad_;
        ...
        class level_formatter final : public flag_formatter {
          public:
            ...
            void format(const details::log_msg &msg, const std::tm &,
                        memory_buf_t &dest) override {
                const string_view_t &level_name =
                    level::to_string_view(msg.level);
        ...
        class e_formatter final : public flag_formatter {
          public:
            ...
            void format(const details::log_msg &msg, const std::tm &,
                        memory_buf_t &dest) override {
                auto millis =
                    fmt_helper::time_fraction<std::chrono::milliseconds>(
                        msg.time);
        ...
        class f_formatter final : public flag_formatter {
          public:
            ...
            void format(const details::log_msg &msg, const std::tm &,
                        memory_buf_t &dest) override {
                auto micros =
                    fmt_helper::time_fraction<std::chrono::microseconds>(
                        msg.time);
        ...
        class F_formatter final : public flag_formatter {
          public:
            ...
            void format(const details::log_msg &msg, const std::tm &,
                        memory_buf_t &dest) override {
                auto ns = fmt_helper::time_fraction<std::chrono::nanoseconds>(
                    msg.time);
          ...
          public:
            ...
            void format(const details::log_msg &msg, const std::tm &,
                ...
                const auto field_size =
                    ScopedPadder::count_digits(msg.thread_id);
                ScopedPadder p(field_size, padinfo_, dest);
        ...
        // mark the color range. expect it to be in the form of "%^colored
        // text%$"
...
#pragma warning(disable : 4127) // consider using 'if constexpr' instead
            ...
            static const char *basename(const char *filename) {
                // if the size is 2 (1 character + null terminator) we can use
                // the more efficient strrchr the branch will be elided by
                // optimizations
                if (sizeof(os::folder_seps) == 2) {
                    ...
                    const auto it = std::find_first_of(
                        begin, end, std::begin(os::folder_seps),
        ...
        class elapsed_formatter final : public flag_formatter {
          public:
            ...
            void format(const details::log_msg &msg, const std::tm &,
                ...
                auto delta_units =
                    std::chrono::duration_cast<DurationUnits>(delta);
                last_message_time_ = msg.time;
                auto delta_count = static_cast<size_t>(delta_units.count());
                auto n_digits = static_cast<size_t>(
                    ScopedPadder::count_digits(delta_count));
...
// some message
          ...
          public:
            ...
            void format(const details::log_msg &msg, const std::tm &tm_time,
                ...
                if (cache_timestamp_ != secs || cached_datetime_.size() == 0) {
                    ...
                    fmt_helper::append_int(tm_time.tm_year + 1900,
                                           cached_datetime_);
                    cached_datetime_.push_back('-');
                ...
                msg.color_range_start = dest.size();
                // fmt_helper::append_string_view(level::to_c_str(msg.level),
                // dest);
                fmt_helper::append_string_view(level::to_string_view(msg.level),
                                               dest);
                msg.color_range_end = dest.size();
                dest.push_back(']');
                dest.push_back(' ');
                ...
                if (!msg.source.empty()) {
                    ...
                    const char *filename = details::short_filename_formatter<
                        details::null_scoped_padder>::basename(msg.source
                                                                   .filename);
                    fmt_helper::append_string_view(filename, dest);
                    dest.push_back(':');
                    fmt_helper::append_int(msg.source.line, dest);
    ...
    SPDLOG_INLINE pattern_formatter::pattern_formatter(
        ...
        : pattern_(std::move(pattern)), eol_(std::move(eol)),
          pattern_time_type_(time_type), need_localtime_(false),
    ...
    SPDLOG_INLINE
    pattern_formatter::pattern_formatter(pattern_time_type time_type,
                                         std::string eol)
