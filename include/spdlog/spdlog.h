...
namespace spdlog {
    ...
    //
    // Useful for initializing manually created loggers with the global
    // settings.
    //
    // Example:
    ...
    // Determine whether the default logger should log messages with a certain
    // level
    ...
    //
    // The default logger object can be accessed using the
    // spdlog::default_logger(): For example, to add another sink to it:
    // spdlog::default_logger()->sinks().push_back(some_sink);
    //
    // The default logger can replaced using
    // spdlog::set_default_logger(new_logger). For example, to replace it with a
    // file logger.
    //
    // IMPORTANT:
    // The default API is thread safe (for _mt loggers), but:
    // set_default_logger() *should not* be used concurrently with the default
    // API. e.g do not call set_default_logger() from one thread while calling
    ...
    inline void log(source_loc source, level::level_enum lvl,
        ...
        default_logger_raw()->log(source, lvl, fmt,
